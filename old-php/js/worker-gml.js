    ...
    if (initial === true && state.tokens.curr.fud) {
      ...
      while ((rbp < state.tokens.next.lbp || state.tokens.next.type === "(template)") &&
        ...
        if (left && (left.value || (left.first && left.first.value))) {
          if (left.first && left.first.value && left.first.value === ".") {
            isArray = false;
    ...
    if (left.id === ".") {
      ...
      if (allowDestructuring && state.tokens.curr.left.destructAssign) {
        ...
        if (left.id === "{" || !left.left) {
          warning("E031", assignToken);
        } else if (left.left.value === "arguments") {
          warning("E031", assignToken);
        }
      }
  ...
  function destructuringPatternRecursive(options) {
    ...
    var nextInnerDE = function() {
      ...
      if (checkPunctuators(state.tokens.next, ["[", "{"])) {
        ...
        if (isAssignment) {
          var identifierToken = state.tokens.next;
...
define("ace/mode/gml_worker",["require","exports","module","ace/lib/oop","ace/worker/mirror","ace/mode/javascript/jshint"], function(require, exports, module) {
"use strict";
